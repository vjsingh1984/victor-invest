...
class PriceTargetChart(Flowable):
    ...
    def draw(self):
        ...
        if self.stop:
            prices.append(self.stop)
        if self.support:
            prices.append(self.support)
        if self.resistance:
            prices.append(self.resistance)
...
class ProfessionalReportGenerator:
    ...
    def _build_story(self, data: Dict) -> List:
        ...
        story.append(KeepTogether([RecommendationBadge(120, 50, rec, conf)]))
        ...
        if current:
            ...
            story.append(PriceTargetChart(400, 100, current, target, stop))
        ...
        if valuation_models:
            ...
            for model_name, model_data in valuation_models.items():
                if isinstance(model_data, dict):
                    ...
                    upside = (
                        model_data.get("upside_percent")
                        or model_data.get("upside_downside_pct")
                        or model_data.get("upside_pct")
                    )
