...
class PDFReportGenerator:
    ...
    def generate_report(self, recommendations: List[Dict], 
        ...
        time_horizon_data = [
            ...
            ['SHORT_TERM', '3-12 months', 'Tactical positions based on near-term catalysts, earnings, or technical patterns. Suitable for active trading strategies.'],
            ['MEDIUM_TERM', '1-3 years', 'Strategic positions based on business fundamentals, industry trends, and moderate growth expectations. Balanced risk-reward profile.'],
            ['LONG_TERM', '3+ years', 'Investment positions based on long-term competitive advantages, secular trends, and compound growth potential. Lower volatility tolerance required.']
        ]
        
        time_horizon_table = Table(time_horizon_data, colWidths=[1.5*inch, 1.2*inch, 4*inch])
        time_horizon_table.setStyle(TableStyle([
        ...
        position_size_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
    ...
    def _create_symbol_analysis(self, recommendation: Dict, include_charts: Optional[List[str]] = None) -> List:
        ...
        if multi_year_trends := recommendation.get('multi_year_trends'):
            if multi_year_metrics := multi_year_trends.get('metrics'):
                ...
                elements.append(Paragraph(f"📈 <b>Multi-Year Historical Trends ({years_analyzed} Years)</b>", self.styles['Heading3']))
                ...
                if cyclical_pattern := multi_year_metrics.get('cyclical_pattern'):
                    pattern_emoji = {'Highly Cyclical': '🔄', 'Moderately Cyclical': '↗️', 'Stable Growth': '📊', 'Volatile': '⚡'}
        ...
        if dcf_valuation := recommendation.get('dcf_valuation'):
            ...
            if enterprise_value := dcf_valuation.get('enterprise_value'):
                ...
                elements.append(Paragraph(f"  • Enterprise Value: ${enterprise_value:.2f}B", self.styles['AnalysisText']))
            ...
            if sensitivity := dcf_valuation.get('sensitivity_analysis'):
                elements.append(Spacer(1, 0.1 * inch))
                elements.append(Paragraph("<b>Sensitivity Analysis - Fair Value Table:</b>", self.styles['AnalysisText']))
            ...
            if crisis_performance := recession_perf.get('crisis_performance'):
                ...
                for crisis_id, crisis_data in crisis_performance.items():
                    ...
                    if revenue_decline := perf.get('revenue_decline_pct'):
                        decline_color = 'green' if revenue_decline >= -5 else ('orange' if revenue_decline >= -15 else 'red')
                        elements.append(Paragraph(
                    ...
                    if quarters != 'N/A':
                        ...
                        elements.append(Paragraph(
                            f"  • Recovery Time: Data incomplete",
                            self.styles['AnalysisText']
                        ))
        ...
        if insider_trading := recommendation.get('insider_trading'):
            ...
            if data_available:
                ...
                if total_transactions > 0:
                    ...
                    elements.append(Paragraph(
                        f"  • Sales: {sell_count} ({100-buy_pct:.0f}%)",
                        self.styles['AnalysisText']
                    ))
                    elements.append(Spacer(1, 0.05 * inch))
                    ...
                    if total_buy_value > 0 or total_sell_value > 0:
                        elements.append(Paragraph(
                            f"  • Total Buy Value: ${total_buy_value:,.0f}",
                            self.styles['AnalysisText']
                        ))
                        elements.append(Paragraph(
                            f"  • Total Sell Value: ${total_sell_value:,.0f}",
                            self.styles['AnalysisText']
                        ))
        ...
        if news_sentiment := recommendation.get('news_sentiment'):
            ...
            if data_available:
                ...
                if article_count > 0:
                    elements.append(Paragraph(
                        f"<b>Article Breakdown</b> ({article_count} total articles):",
                        self.styles['AnalysisText']
                    ))
                    elements.append(Spacer(1, 0.05 * inch))
        ...
        if risk_scores := recommendation.get('risk_scores'):
            ...
            elements.append(Paragraph(f"<b>Overall Risk Score:</b> <font color='{color}' size='12'>{overall_risk}/10 ({risk_rating})</font>",
                                    self.styles['AnalysisText']))
            elements.append(Spacer(1, 0.1 * inch))
            ...
            if financial_risk := risk_scores.get('financial_health_risk'):
                ...
                risk_items.append(f"<b>Financial Health:</b> <font color='{risk_color}'>{financial_risk}/10</font> - Leverage, liquidity, solvency")

            # Market Risk
            if market_risk := risk_scores.get('market_risk'):
                risk_color = 'green' if market_risk < 4 else ('orange' if market_risk < 7 else 'red')
                risk_items.append(f"<b>Market Risk:</b> <font color='{risk_color}'>{market_risk}/10</font> - Volatility, beta, drawdowns")
            ...
            if operational_risk := risk_scores.get('operational_risk'):
                ...
                risk_items.append(f"<b>Operational Risk:</b> <font color='{risk_color}'>{operational_risk}/10</font> - Cash flow quality, working capital")
            ...
            if business_risk := risk_scores.get('business_model_risk'):
                ...
                risk_items.append(f"<b>Business Model Risk:</b> <font color='{risk_color}'>{business_risk}/10</font> - Margin stability, competitive moat")

            # Growth Risk
            if growth_risk := risk_scores.get('growth_risk'):
                risk_color = 'green' if growth_risk < 4 else ('orange' if growth_risk < 7 else 'red')
                risk_items.append(f"<b>Growth Risk:</b> <font color='{risk_color}'>{growth_risk}/10</font> - Growth sustainability, capital efficiency")
        ...
        if peer_leaderboard := recommendation.get('peer_leaderboard'):
            if peers := peer_leaderboard.get('peers'):
                ...
                elements.append(Paragraph(f"📊 <b>Peer Performance Leaderboard - {industry}</b>", self.styles['Heading3']))
                ...
                if target_peer:
                    ...
                    elements.append(Paragraph(f"<b>Overall Rank:</b> <font color='{rank_color}'>#{overall_rank} of {total_peers}</font> ({percentile}th percentile)",
                                            self.styles['AnalysisText']))
                    elements.append(Spacer(1, 0.1 * inch))
                ...
                leaderboard_table = Table(table_data, colWidths=[0.6*inch, 0.9*inch, 1.0*inch, 1.1*inch, 0.8*inch, 1.1*inch])
        ...
        if chart_patterns := recommendation.get('chart_patterns'):
            if chart_patterns.get('pattern_count', 0) > 0:
                ...
                pattern_table = Table(pattern_data, colWidths=[1.5*inch, 1.0*inch, 0.9*inch, 1.1*inch, 1.0*inch])
    ...
    def _create_charts_section(self, chart_paths: List[str]) -> List:
        ...
        for chart_path in chart_paths:
            if 'score_history' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("Track how our investment assessment has evolved over time", self.styles['Normal']))
        ...
        for chart_path in chart_paths:
            if 'valuation_comparison' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("Comparison of valuation multiples against peer group median", self.styles['Normal']))
                img = Image(chart_path, width=6*inch, height=4*inch)
                elements.append(img)
                elements.append(Spacer(1, 0.3 * inch))
                break
        ...
        for chart_path in chart_paths:
            if 'revenue_trend' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("Revenue trend showing quarter-over-quarter growth rates", self.styles['Normal']))
        ...
        for chart_path in chart_paths:
            if 'cash_flow' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("Operating cash flow with quarter-over-quarter growth", self.styles['Normal']))
                img = Image(chart_path, width=6.5*inch, height=4*inch)
                elements.append(img)
                elements.append(Spacer(1, 0.3 * inch))
                break
        ...
        for chart_path in chart_paths:
            if 'multi_year_trends' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("Multi-year revenue and earnings trends with CAGR analysis", self.styles['Normal']))
        ...
        for chart_path in chart_paths:
            if 'risk_radar' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("5-dimensional risk analysis across financial health, market, operational, business model, and growth factors", self.styles['Normal']))
        ...
        for chart_path in chart_paths:
            if 'competitive_positioning' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("Competitive position relative to industry peers based on revenue growth and profit margins", self.styles['Normal']))
        ...
        for chart_path in chart_paths:
            if 'volume_profile' in chart_path and Path(chart_path).exists():
                ...
                elements.append(Paragraph("Volume distribution across price levels showing Point of Control (POC) and value area (70% of volume)", self.styles['Normal']))
                img = Image(chart_path, width=7*inch, height=6*inch)
                elements.append(img)
                elements.append(Spacer(1, 0.3 * inch))
                break
    ...
    def _create_entry_exit_section(self, recommendation: Dict) -> List:
        ...
        if optimal_entry_zone and current_price > 0:
            ...
            zone_data.append(['Recommended Allocation', f"{optimal_entry_zone.get('recommended_allocation_pct', 0):.1f}%"])
        ...
        if entry_signals:
            ...
            entry_table = Table(entry_data, colWidths=[1.3 * inch, 0.9 * inch, 0.9 * inch, 0.9 * inch, 0.7 * inch, 0.9 * inch])
