...
class SECCompanyFactsExtractor:
    ...
    def _get_fiscal_year_end_month(self, cik: str) -> Optional[int]:
        ...
        Returns:
            Fiscal year end month (1-12) or None if not found

        Example:
            ORCL (cik=1341439) → 5 (May fiscal year end)
        ...
        try:
            ...
            if result:
                fy_end_month = int(result.fy_end_month)
                logger.debug(f"Fiscal year end month for CIK {cik}: {fy_end_month}")
                return fy_end_month
            else:
                logger.debug(f"No fiscal year end data found for CIK {cik}")
                return None

        except Exception as e:
            logger.warning(f"Error fetching fiscal year end month for CIK {cik}: {e}")
            return None
    ...
    def _calculate_fiscal_year_from_date(
        ...
        Uses centralized FiscalPeriodService for authoritative calculation.
        Handles all edge cases including Jan-Jun fiscal year ends.
    ...
    def _determine_latest_fiscal_period(
        ...
        try:
            ...
            # ENHANCED: Validate fiscal_year for non-calendar fiscal year companies
            all_periods = set()
            ...
            if cik:
                ...
                if fiscal_year_end_month and fiscal_year_end_month != 12:
                    logger.info(
                        ...
                        f"(fiscal year end month: {fiscal_year_end_month}). "
            ...
            for tag_name, tag_data in us_gaap.items():
                ...
                for entry in usd_data:
                    ...
                    if fy and fp and filed and end_date:
                        # Validate fiscal_year for non-calendar fiscal year companies
                        if fiscal_year_end_month and fiscal_year_end_month != 12:
                            # Recalculate fiscal_year from end date
                            # Pass fiscal_period to help disambiguate edge cases
                            calculated_fy = self._calculate_fiscal_year_from_date(
                                end_date, fiscal_year_end_month, fiscal_period=fp
                            )

                            # If API provides wrong fiscal_year, override it
                            if calculated_fy != fy:
                                logger.info(
                                    f"[DEBUG_FY_FIX] {symbol} - Fiscal year mismatch for period {end_date}: "
                                    f"API={fy}, Calculated={calculated_fy} (fiscal_year_end_month={fiscal_year_end_month}). "
    ...
    def extract_financial_metrics(self, symbol: str) -> Dict:
        ...
        try:
            ...
            # =====================================================
            # SHARES OUTSTANDING - Critical for per-share valuations
            ...
            dei_shares = None
            if dei:
                dei_shares_concept = dei.get("EntityCommonStockSharesOutstanding", {})
            ...
            # =====================================================
            dividends_paid = get_metric(None, canonical_name="dividends_paid")
            ...
            return {
                ...
                "unbilled_contracts_receivable": unbilled_contracts_receivable,
                # Metadata
                "symbol": symbol,
