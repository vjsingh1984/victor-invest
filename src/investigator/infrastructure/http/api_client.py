            ...
            if hasattr(ollama_config, 'model_specs'):
                ...
                for spec_model, spec in model_specs.items():
                    if spec_model in model.lower() or model.lower() in spec_model:
                        if hasattr(spec, 'context_window'):
                            logger.debug(f"Using config.json context size {spec.context_window} for model {model} (matched {spec_model})")
                            return spec.context_window

        # PRIORITY 2: Fallback to hardcoded dict for backward compatibility
        model_contexts = {
            'llama3.1': 131072, 'llama-3.1': 131072,
            'llama3.3': 131072, 'llama-3.3': 131072,
            'deepseek-r1:32b': 131072, 'deepseek-r1': 131072,
            'qwen2.5:32b': 32768, 'qwen3-30b-40k': 40960,
            'qwen3:30b': 262144, 'qwen3:32b': 40960, 'qwen3-coder:30b': 262144,
            'phi4-reasoning:plus': 32768, 'phi-4': 16384, 'phi3': 4096,
            'mixtral-8x7b': 32768, 'mixtral:8x7b': 32768,
            'mistral:7b': 8192, 'mistral:latest': 8192,
        }
