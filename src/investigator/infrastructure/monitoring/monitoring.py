...
class MetricsCollector:
    ...
    def __init__(self, export_interval: int = 60):
        ...
        # Agent metrics
        self.agent_metrics = {"executions": {}, "failures": {}, "durations": {}, "last_execution": {}}
    ...
    def _init_prometheus_metrics(self):
        ...
        self.analysis_counter = get_or_create_metric(
            Counter, "investigator_analyses_total", "Total number of analyses performed", ["symbol", "mode", "status"]
        )

        self.agent_execution_counter = get_or_create_metric(
            Counter, "investigator_agent_executions_total", "Total agent executions", ["agent_type", "status"]
        )

        self.cache_counter = get_or_create_metric(
            Counter, "investigator_cache_operations_total", "Cache operations", ["operation", "tier"]
        )
        ...
        self.system_cpu_gauge = get_or_create_metric(
            Gauge, "investigator_system_cpu_percent", "System CPU usage percentage"
        )

        self.system_memory_gauge = get_or_create_metric(
            Gauge, "investigator_system_memory_percent", "System memory usage percentage"
        )

        # Summaries
        self.llm_latency_summary = get_or_create_metric(
            Summary, "investigator_llm_latency_seconds", "LLM response latency", ["model"]
        )
    ...
    def record_llm_latency(self, model: str, latency: float):
        ...
        self._add_metric(
            MetricPoint(name="llm_latency", value=latency, labels={"model": model}, metric_type=MetricType.HISTOGRAM)
        )
    ...
    def record_orchestrator_stats(self, stats: Dict):
        ...
        for key, value in stats.items():
            if isinstance(value, (int, float)):
                self._add_metric(MetricPoint(name=f"orchestrator_{key}", value=value, metric_type=MetricType.GAUGE))
    ...
    def _serialize_agent_metrics(self) -> Dict:
        ...
        for agent_type in self.agent_metrics["executions"].keys():
            ...
            serialized[agent_type] = {
                ...
                "last_execution": (
                    self.agent_metrics["last_execution"].get(agent_type, "").isoformat()
                    if agent_type in self.agent_metrics["last_execution"]
                    else None
                ),
            }
    ...
    async def check_alerts(self):
        ...
        if health["metrics"]["cpu_percent"] > self.thresholds["cpu_critical"]:
            await self._raise_alert(
                "cpu_critical", f"CPU usage critical: {health['metrics']['cpu_percent']:.1f}%", "critical"
            )
        elif health["metrics"]["cpu_percent"] > self.thresholds["cpu_warning"]:
            await self._raise_alert(
                "cpu_warning", f"CPU usage high: {health['metrics']['cpu_percent']:.1f}%", "warning"
            )
        else:
        ...
        if health["metrics"]["memory_percent"] > self.thresholds["memory_critical"]:
            await self._raise_alert(
                "memory_critical", f"Memory usage critical: {health['metrics']['memory_percent']:.1f}%", "critical"
            )
        elif health["metrics"]["memory_percent"] > self.thresholds["memory_warning"]:
            await self._raise_alert(
                "memory_warning", f"Memory usage high: {health['metrics']['memory_percent']:.1f}%", "warning"
            )
        else:
