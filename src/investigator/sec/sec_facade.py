            ...
            for qd in quarterly_data:
                try:
                    ...
                    quarterly_data_dict = {
                        ...
                        'total_debt': balance_sheet.get('long_term_debt', 0),  # Use long_term_debt as proxy for total_debt
                    ...
                    if success:
                        ...
                        self.logger.warning(f"Failed to cache quarterly metrics for {symbol} {fiscal_year}-{fiscal_period}")
                        
                except Exception as e:
                    self.logger.warning(f"Failed to calculate metrics for {symbol} period {getattr(qd, 'fiscal_period', 'unknown')}: {e}")
