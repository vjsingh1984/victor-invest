...
class ValuationFrameworkPlanner:
    ...
    def classify_company_stage(self, revenue_growth_pct: float, fcf_margin_pct: float) -> str:
        ...
        if (
            ...
            and revenue_growth_pct > 30.0
            and fcf_margin_pct > 20.0
            and self.market_cap_billions < self.SIZE_LARGE_CAP  # < $10B
        ):
            logger.info(
        ...
        if (
            self.sector in ["Technology", "Communication Services"]
            and 15.0 <= revenue_growth_pct <= 40.0
            and 10.0 <= fcf_margin_pct <= 35.0
        ):
            logger.info(
        ...
        if (
            ...
            and 10.0 <= revenue_growth_pct <= 25.0
            and fcf_margin_pct > 10.0
        ):
            logger.info(
