...
New users should use the Victor-powered CLI instead:
...
The Victor CLI provides:
- StateGraph-based workflows (more reliable)
- Multi-provider support (Anthropic, OpenAI, Ollama, etc.)
- Better resource management
- Cleaner architecture

This legacy CLI will continue to work but is no longer actively developed.
See victor_invest/ for the new architecture.
"""
...
def analyze(ctx, symbol, mode, output, format, detail_level, report, force_refresh, refresh_alias):
    ...
    async def run_analysis():
        ...
        # FIX Issue #2: Pass config path from CLI context to honor --config flag
        ...
        # FIX Issue #5: Use instance overrides instead of mutating global config
        if force_refresh:
            click.echo(f"🔄 Force refresh enabled - clearing all caches for {symbol}")
            cache_manager.set_force_refresh(True, [symbol])
...
def batch(ctx, symbols, mode, output_dir, detail_level, force_refresh, refresh_alias):
    ...
    async def run_batch():
        ...
        cfg = get_config(config_path=ctx.obj.get("config_path"))
        ...
        # FIX Issue #5: Use instance overrides
        if force_refresh:
            cache_manager.set_force_refresh(True, list(symbols))
        metrics_collector = MetricsCollector()
        orchestrator = AgentOrchestrator(cache_manager, metrics_collector)
    ...
    tests = [
        ...
        (
            ...
            [
                "python3",
                "-c",
                "from investigator.infrastructure.database.db import get_database_engine as get_engine; get_engine(); print('OK')",
            ],
        ),
        (
            "Cache system",
