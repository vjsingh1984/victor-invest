...
"""
    ...
    async def _get_recent(
        ...
        total_purchases = sum(
            f.get("total_value", 0) for f in filings
            if f.get("transaction_code") == "P"
        )
        total_sales = sum(
            abs(f.get("total_value", 0)) for f in filings
    ...
    async def _detect_clusters(self, symbol: str, days: int) -> ToolResult:
        """Detect cluster activity."""
        clusters = await self._sentiment_service.detect_cluster_activity(
            symbol, days
        )
    ...
    async def _get_key_insiders(self, symbol: str, days: int) -> ToolResult:
        ...
        summary = await self._sentiment_service.get_key_insider_summary(
            symbol, days
        )
    ...
    async def _fetch_filings(self, symbol: str, days: int) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.error_result(
                f"Failed to fetch filings: {str(e)}",
