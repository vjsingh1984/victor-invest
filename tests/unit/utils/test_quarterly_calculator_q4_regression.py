...
from investigator.domain.services.quarterly_processor import convert_ytd_to_quarterly, get_rolling_ttm_periods
...
class TestQ4ComputationRegression:
    ...
    def test_no_184_day_gaps_after_q4_computation(self, zs_fiscal_periods):
        ...
        **Fix**: With Q4-2023 (Jul 31, 2023) computed:
        ...
        sorted_periods = sorted(result, key=lambda p: datetime.strptime(p["period_end_date"], "%Y-%m-%d"), reverse=True)
    ...
    def test_get_rolling_ttm_periods_includes_q4(self, zs_fiscal_periods):
        ...
        result = get_rolling_ttm_periods(
            quarterly_metrics=zs_fiscal_periods, target_quarters=4, fiscal_year_end_month=7, fiscal_year_end_day=31
        )
        ...
        assert "Q4" in periods, f"TTM periods should include Q4, got: {periods}"

        # Should have 4 consecutive quarters (including Q4)
        assert len(result) >= 4, f"Expected at least 4 quarters, got {len(result)}"
